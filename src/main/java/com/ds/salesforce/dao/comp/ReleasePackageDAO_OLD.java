...
public class ReleasePackageDAO_OLD implements ISFBaseDAO {
	...
	public List<Object> findById(String id, SFoAuthHandle sfHandle) {
		...
		com.sforce.soap.enterprise.sobject.ASA__ReleasePackage__c retObj = null;
		...
		try {
			...
			if (queryResults.getSize() > 0) {
				...
				for (int i = 0; i < queryResults.getRecords().length; i++) {
					retObj = (com.sforce.soap.enterprise.sobject.ASA__ReleasePackage__c) queryResults
							.getRecords()[i];
					...
					System.out.println(" - Relase: " + retObj.getASA__Release__c());
					System.out.println(" --------------: ");
				}
			} else {
				System.out
		...
		try {
			...
			if (queryResults.getSize() > 0) {
				for (int i = 0; i < queryResults.getRecords().length; i++) {
					...
					System.out.println(" - Order: " + retObj.getASA__Order__c());
					System.out.println(" - Package: " + retObj.getASA__Package__c());
					Double order = new Double(retObj.getASA__Order__c());
					if(retObj.getASA__Package__c() != null){
						PackageDAO pkgDAO = new PackageDAO();
						List<Object> packageDOList = pkgDAO.findById(retObj.getASA__Package__c(), sfHandle);
