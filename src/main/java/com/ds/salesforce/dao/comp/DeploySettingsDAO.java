...
public class DeploySettingsDAO implements ISFBaseDAO {
	...
	public boolean insert(Object obj, SFoAuthHandle sfHandle) {
		...
		ASA__DeploymentSetting__c[] record = new ASA__DeploymentSetting__c[1];
		try {
			// Get the name of the sObject
			ASA__DeploymentSetting__c a = new ASA__DeploymentSetting__c();
			a.setASA__BaseOrganizationId__c(deploySettingsDO.getOrgId());
			a.setASA__Server_URL__c(deploySettingsDO.getServerUrl());
			a.setASA__TokenCode__c(deploySettingsDO.getToken());
			a.setASA__RefreshTokenCode__c(deploySettingsDO.getRefreshToken());
			record[0] = a;
	...
	public boolean update(Object obj, SFoAuthHandle sfHandle) {
		// TODO Auto-generated method stub
		com.sforce.soap.enterprise.sobject.ASA__DeploymentSetting__c newEnvObj = new com.sforce.soap.enterprise.sobject.ASA__DeploymentSetting__c();
		...
		try {
			...
			newEnvObj.setASA__BaseOrganizationId__c(dsDO.getOrgId());
			newEnvObj.setASA__TokenCode__c(dsDO.getToken());
			newEnvObj.setASA__Server_URL__c(dsDO.getServerUrl());
			newEnvObj.setASA__RefreshTokenCode__c(dsDO.getRefreshToken());
			com.sforce.soap.enterprise.sobject.ASA__DeploymentSetting__c[] mobj = new com.sforce.soap.enterprise.sobject.ASA__DeploymentSetting__c[1];
			mobj[0] = newEnvObj;
			...
			if (queryResults.getSize() > 0) {
				for (int i = 0; i < queryResults.getRecords().length; i++) {
					...
					System.out.println(" - BaseOrg: "
							+ retObj.getASA__BaseOrganizationId__c());
					System.out.println(" - token: " + retObj.getASA__TokenCode__c());
