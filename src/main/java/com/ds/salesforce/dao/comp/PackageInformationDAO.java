...
import com.sforce.soap.enterprise.sobject.ASAClient__PackageInformation__c;
...
public class PackageInformationDAO implements ISFBaseDAO {
		...
		try {
			...
			if (queryResults.getSize() > 0) {
				for (int i = 0; i < queryResults.getRecords().length; i++) {
					...
					System.out.println(" - Name : " + locObj.getName());
					System.out
							.println(" - desc: " + locObj.getASAClient__Description__c());
					System.out.println(" - release Id: "
							+ locObj.getASAClient__Release__c());
					System.out.println(" - release Name: "
							+ locObj.getASAClient__Release__r().getName());
					...
					retObj = new PackageInformationDO(locObj.getId(),
							locObj.getName(), locObj.getASAClient__Description__c(),
							locObj.getASAClient__Release__c(), locObj.getASAClient__ReadyForDeployment__c(),locObj.getASAClient__Package_Retrieved_Time__c());
	...
	public boolean insert(Object obj, SFoAuthHandle sfHandle) {
		...
		ASAClient__PackageInformation__c[] record = new ASAClient__PackageInformation__c[1];
		try {
			...
			for (Iterator iterator = relInfoList.iterator(); iterator.hasNext();) {
				...
				ASAClient__PackageInformation__c a = new ASAClient__PackageInformation__c();
				a.setName(pkgInfoDO.getName());
				a.setASAClient__Description__c(pkgInfoDO.getDescription());
				a.setASAClient__Release__c(relInfoDO.getId());
				record[0] = a;
