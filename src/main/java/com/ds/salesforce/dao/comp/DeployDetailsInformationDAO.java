...
import com.sforce.soap.enterprise.sobject.ASAClient__DeploymentDetailsInformation__c;
...
public class DeployDetailsInformationDAO implements ISFBaseDAO {
	...
	public boolean insert(Object obj, SFoAuthHandle sfHandle) {
		...
		ASAClient__DeploymentDetailsInformation__c[] record = new ASAClient__DeploymentDetailsInformation__c[1];
		try {
			// Get the name of the sObject
			ASAClient__DeploymentDetailsInformation__c a = new ASAClient__DeploymentDetailsInformation__c();
			a.setASAClient__MetadataLog__c(errorLogBean.getMetadataLogId());
			a.setASAClient__Error_Message__c(errorLogBean.getErrorMessage());
			a.setASAClient__Name__c(errorLogBean.getName() + " for Org: "
					+ errorLogBean.getOrgId());
			a.setASAClient__Type__c(errorLogBean.getType());
			record[0] = a;
