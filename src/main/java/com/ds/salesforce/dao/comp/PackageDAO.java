...
import com.sforce.soap.enterprise.sobject.ASA__PackageComponent__c;
import com.sforce.soap.enterprise.sobject.ASA__Package__c;
...
public class PackageDAO implements ISFBaseDAO {
	...
	public boolean insert(Object obj, SFoAuthHandle sfHandle) {
		...
		ASA__Package__c[] record = new ASA__Package__c[1];
		try {
			// Get the name of the sObject
			ASA__Package__c a = new ASA__Package__c();
			//a.setId(relDO.getId());
			a.setName(pkgDO.getName());
			a.setASA__Description__c(pkgDO.getDescription());
			record[0] = a;
	...
	public String insertAndGetId(Object obj, SFoAuthHandle sfHandle) {
		...
		try {
			...
			a.setName(pkgDO.getName());
			a.setASA__Description__c(pkgDO.getDescription());
			a.setASA__ParentPackageID__c(pkgDO.getParentPackageId());
			record[0] = a;
	...
	public List<Object> findById(String id, SFoAuthHandle sfHandle) {
		com.sforce.soap.enterprise.sobject.ASA__Package__c package__c = null;
		...
		try {
			...
			if (queryResults.getSize() > 0) {
				...
				for (int i = 0; i < queryResults.getRecords().length; i++) {
					package__c = (com.sforce.soap.enterprise.sobject.ASA__Package__c) queryResults
							.getRecords()[i];

					packageDO = new PackageDO(package__c.getId(),
							package__c.getName(),package__c.getASA__Description__c(),package__c.getASA__ParentPackageID__c());
