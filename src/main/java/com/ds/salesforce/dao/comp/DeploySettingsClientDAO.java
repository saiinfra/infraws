...
public class DeploySettingsClientDAO implements ISFBaseDAO {
	...
	public boolean insert(Object obj, SFoAuthHandle sfHandle) {
		...
		ASAClient__DeploymentSettingClient__c[] record = new ASAClient__DeploymentSettingClient__c[1];
		try {
			// Get the name of the sObject
			ASAClient__DeploymentSettingClient__c a = new ASAClient__DeploymentSettingClient__c();
			a.setASAClient__BaseOrganizationId__c(deploySettingsDO.getOrgId());
			a.setASAClient__Server_URL__c(deploySettingsDO.getServerUrl());
			a.setASAClient__TokenCode__c(deploySettingsDO.getToken());
			a.setASAClient__RefreshTokenCode__c(deploySettingsDO.getRefreshToken());
			record[0] = a;
	...
	public boolean update(Object obj, SFoAuthHandle sfHandle) {
		// TODO Auto-generated method stub
		com.sforce.soap.enterprise.sobject.ASAClient__DeploymentSettingClient__c newEnvObj = new com.sforce.soap.enterprise.sobject.ASAClient__DeploymentSettingClient__c();
		...
		try {
			...
			newEnvObj.setASAClient__BaseOrganizationId__c(dsDO.getOrgId());
			newEnvObj.setASAClient__TokenCode__c(dsDO.getToken());
			newEnvObj.setASAClient__Server_URL__c(dsDO.getServerUrl());
			newEnvObj.setASAClient__RefreshTokenCode__c(dsDO.getRefreshToken());
			com.sforce.soap.enterprise.sobject.ASAClient__DeploymentSettingClient__c[] mobj = new com.sforce.soap.enterprise.sobject.ASAClient__DeploymentSettingClient__c[1];
			mobj[0] = newEnvObj;
	...
	public List<Object> findById(String orgId, SFoAuthHandle sfHandle) {
		// TODO Auto-generated method stub
		com.sforce.soap.enterprise.sobject.ASAClient__DeploymentSettingClient__c retObj = null;
		...
		try {
			...
			if (queryResults.getSize() > 0) {
				for (int i = 0; i < queryResults.getRecords().length; i++) {
					...
					retObj = (com.sforce.soap.enterprise.sobject.ASAClient__DeploymentSettingClient__c) queryResults
							.getRecords()[i];
					dsDO = new DeploymentSettingClientDO(retObj.getId(),
							retObj.getASAClient__BaseOrganizationId__c(),
							retObj.getASAClient__TokenCode__c(),
							retObj.getASAClient__Server_URL__c(),
							retObj.getASAClient__RefreshTokenCode__c());
					System.out.println(" - Id: " + retObj.getId());
					System.out.println(" - BaseOrg: "
							+ retObj.getASAClient__BaseOrganizationId__c());
					System.out.println(" - token: " + retObj.getASAClient__TokenCode__c());
