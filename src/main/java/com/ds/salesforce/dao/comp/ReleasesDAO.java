...
import com.sforce.soap.enterprise.sobject.ASA__ReleasePackage__c;
import com.sforce.soap.enterprise.sobject.ASA__Releases__c;
...
public class ReleasesDAO implements ISFBaseDAO {
		...
		try {
			...
			if (queryResults.getSize() > 0) {
				for (int i = 0; i < queryResults.getRecords().length; i++) {
					...
					com.sforce.soap.enterprise.sobject.ASA__Releases__c locObj = (com.sforce.soap.enterprise.sobject.ASA__Releases__c) queryResults
							.getRecords()[i];

					System.out.println(" - Id: " + locObj.getId());
					System.out.println(" - Name: " + locObj.getName());
					System.out.println(" - Status: " + locObj.getASA__Status__c());

					retObj = new ReleasesDO(locObj.getId(), locObj.getName(),
							locObj.getASA__Status__c());
					list.add(retObj);
				}
			} else {
	...
	public boolean insert(Object obj, SFoAuthHandle sfHandle) {
		...
		ASA__Releases__c[] record = new ASA__Releases__c[1];
		try {
			// Get the name of the sObject
			ASA__Releases__c a = new ASA__Releases__c();
			a.setId(relDO.getId());
			a.setASA__Status__c(relDO.getStatus());
	...
	public boolean update(Object obj, SFoAuthHandle sfHandle) {
		com.sforce.soap.enterprise.sobject.ASA__Releases__c newRelObj = new com.sforce.soap.enterprise.sobject.ASA__Releases__c();
		ReleasesDO lObj = (ReleasesDO) obj;
		try {
			newRelObj.setId(lObj.getId());
			newRelObj.setName(lObj.getName());
			newRelObj.setASA__Status__c(lObj.getStatus());
			com.sforce.soap.enterprise.sobject.ASA__Releases__c[] mobj = new com.sforce.soap.enterprise.sobject.ASA__Releases__c[1];
			mobj[0] = newRelObj;
