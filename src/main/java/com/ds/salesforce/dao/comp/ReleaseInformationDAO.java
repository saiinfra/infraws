...
public class ReleaseInformationDAO implements ISFBaseDAO {
		...
		try {
			...
			if (queryResults.getSize() > 0) {
				for (int i = 0; i < queryResults.getRecords().length; i++) {
					...
					com.sforce.soap.enterprise.sobject.ASAClient__ReleaseInformation__c locObj = (com.sforce.soap.enterprise.sobject.ASAClient__ReleaseInformation__c) queryResults
							.getRecords()[i];
	...
	public boolean insert(Object obj, SFoAuthHandle sfHandle) {
		...
		ASAClient__ReleaseInformation__c[] record = new ASAClient__ReleaseInformation__c[1];
		try {
			// Get the name of the sObject
			ASAClient__ReleaseInformation__c a = new ASAClient__ReleaseInformation__c();
			// a.setId(relDO.getId());
			a.setASAClient__Status__c(relDO.getStatus());
			a.setASAClient__ParentReleaseID__c(relDO.getParentReleaseID());
	...
	public boolean update(Object obj, SFoAuthHandle sfHandle) {
		com.sforce.soap.enterprise.sobject.ASAClient__ReleaseInformation__c newRelObj = new com.sforce.soap.enterprise.sobject.ASAClient__ReleaseInformation__c();
