...
public class DeployMetadataDAO implements ISFBaseDAO {
	...
	public List<Object> findById(String metadataLogIdName,
		...
		com.sforce.soap.enterprise.sobject.ASA__DeployMetadata__c retObj = null;
		...
		try {
			...
			if (queryResults.getSize() > 0) {
				...
				for (int i = 0; i < queryResults.getRecords().length; i++) {
					retObj = (com.sforce.soap.enterprise.sobject.ASA__DeployMetadata__c) queryResults
							.getRecords()[i];
					System.out.println(retObj.getASA__Order__c());
					
					System.out.println(" - Name: " + retObj.getASA__Name__c());
					System.out.println(" - Type: " + retObj.getASA__Type__c());
					...
					if(retObj.getASA__Package__c() != null){
						PackageDAO dao = new PackageDAO();
						List<Object> packageDOList = dao.findById(retObj.getASA__Package__c(), sfHandle);
					...
					System.out.println(" - metadata log: "
							+ retObj.getASA__MetadataLog__r().getName());
					...
					metaBean = new MetaBean(retObj.getId(),
							retObj.getASA__Name__c(), retObj.getASA__Type__c(),
							retObj.getASA__SourceOrganizationId__c(),
							retObj.getASA__OrganizationId__c(), new Double(retObj.getASA__Order__c()), packageName);
