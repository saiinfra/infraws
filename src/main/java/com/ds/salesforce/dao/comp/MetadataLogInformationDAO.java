...
public class MetadataLogInformationDAO implements ISFBaseDAO {
	...
	public boolean update(Object obj, SFoAuthHandle sfHandle) {
		try {
			...
			com.sforce.soap.enterprise.sobject.ASAClient__MetadataLogInformation__c metadataLogInformation__c = new com.sforce.soap.enterprise.sobject.ASAClient__MetadataLogInformation__c();
			...
			if (metadataLogInformationDOobj instanceof MetadataLogInformationDO) {
				...
				metadataLogInformation__c.setASAClient__Status__c(metadataLogInformationDOobj.getStatus());
			}

			SaveResult[] saveResults = sfHandle
					.getEnterpriseConnection()
					.update(new com.sforce.soap.enterprise.sobject.ASAClient__MetadataLogInformation__c[] { metadataLogInformation__c });
	...
	public List<Object> findById1(String metadataLogId, SFoAuthHandle sfHandle) {
		com.sforce.soap.enterprise.sobject.ASAClient__MetadataLogInformation__c metadataLogInformation__c = null;
		...
		try {
			...
			if (queryResults.getSize() > 0) {
				...
				for (int i = 0; i < queryResults.getRecords().length; i++) {
					metadataLogInformation__c = (com.sforce.soap.enterprise.sobject.ASAClient__MetadataLogInformation__c) queryResults
							.getRecords()[i];

					
					metadataDO = new MetadataLogInformationDO(metadataLogInformation__c.getId(),
							metadataLogInformation__c.getASAClient__BaseOrgId__c(),
							metadataLogInformation__c.getASAClient__BaseOrgToken__c(),
							metadataLogInformation__c.getASAClient__BaseOrgUrl__c(),
							metadataLogInformation__c.getASAClient__OrganizationId__c(),
							metadataLogInformation__c.getName(),
							metadataLogInformation__c.getASAClient__Action__c(),
							metadataLogInformation__c.getASAClient__Status__c(), null);
					...
					System.out.println(" - Status: "
							+ metadataLogInformation__c.getASAClient__Status__c());
			...
			if (queryResults.getSize() > 0) {
				...
				for (int i = 0; i < queryResults.getRecords().length; i++) {
					...
					metadataDO = new MetadataLogInformationDO(metadataLogInformation__c.getId(),
							...
							metadataLogInformation__c.getName(),
							metadataLogInformation__c.getASAClient__Action__c(),
							metadataLogInformation__c.getASAClient__Status__c(), noOfPackgsByOrderMap);
					list.add(metadataDO);
				}
			} else {
				System.out.println(" There are no records size is - : "
