...
public class MetadataLogDAO implements ISFBaseDAO {
	...
	public boolean update(Object obj, SFoAuthHandle sfHandle) {
		try {
			...
			com.sforce.soap.enterprise.sobject.ASA__MetadataLog__c metadataLog__c = new com.sforce.soap.enterprise.sobject.ASA__MetadataLog__c();

			if (metadataLogDOobj instanceof MetadataLogDO) {
				metadataLog__c.setId(metadataLogDOobj.getId());
				metadataLog__c.setASA__Status__c(metadataLogDOobj.getStatus());
			}

			SaveResult[] saveResults = sfHandle
					.getEnterpriseConnection()
					.update(new com.sforce.soap.enterprise.sobject.ASA__MetadataLog__c[] { metadataLog__c });
	...
	public List<Object> findById1(String metadataLogId, SFoAuthHandle sfHandle) {
		com.sforce.soap.enterprise.sobject.ASA__MetadataLog__c metadataLog__c = null;
		...
		try {
			...
			if (queryResults.getSize() > 0) {
				...
				for (int i = 0; i < queryResults.getRecords().length; i++) {
					metadataLog__c = (com.sforce.soap.enterprise.sobject.ASA__MetadataLog__c) queryResults
							.getRecords()[i];

					
					metadataDO = new MetadataLogDO(metadataLog__c.getId(),
							metadataLog__c.getASA__BaseOrgId__c(),
							metadataLog__c.getASA__BaseOrgToken__c(),
							metadataLog__c.getASA__BaseOrgUrl__c(),
							metadataLog__c.getASA__OrganizationId__c(),
							metadataLog__c.getName(),
							metadataLog__c.getASA__Action__c(),
							metadataLog__c.getASA__Status__c(), null);
					...
					System.out.println(" - Status: "
							+ metadataLog__c.getASA__Status__c());
			...
			if (queryResults.getSize() > 0) {
				...
				for (int i = 0; i < queryResults.getRecords().length; i++) {
					...
					System.out.println(" - Release: "
							+ metadataLog__c.getASA__Releases__c());
					System.out.println(" - ReleaseEnvironment__c: "
							+ metadataLog__c.getASA__ReleaseEnvironment__c());
					System.out.println(" --------------: ");

					if (metadataLog__c.getASA__Releases__c() != null) {
						System.out.println("releases");
						ReleasePackageDAO dao = new ReleasePackageDAO();
						List<Object> list1 = dao.findByReleaseId(
								metadataLog__c.getASA__Releases__c(), sfHandle);
						/*
					...
					metadataDO = new MetadataLogDO(metadataLog__c.getId(),
							...
							metadataLog__c.getName(),
							metadataLog__c.getASA__Action__c(),
							metadataLog__c.getASA__Status__c(), noOfPackgsByOrderMap);
					list.add(metadataDO);
				}
			} else {
				System.out.println(" There are no records size is - : "
