...
public class RetrieveTask implements Runnable {
	...
	public RetrieveTask(String orgId, String token, String serverURL, String refreshToken,
			String metadataLogId) {
	...
	public void run() {
		...
		try {
			...
			(new FDRetrieveCompService()).retrieve(getOrgId(), getToken(), getServerURL(),getRefreshToken(),
					getMetadataLogId());
		} catch (Exception e) {
			errorFlag = true;
			...
			if(errorFlag){
				System.out.println("Retrieve Operation Complete for requestId: "+getMetadataLogId()+ "\nWith Errors: "+errors);	
			}
			else{
				System.out.println("Retrieve Operation Complete for requestId: "+getMetadataLogId());
			}
