...
@WebServlet(name = "baseoauthclientservlet", urlPatterns = { "/baseoauthclientservlet/*",
		"/baseoauthclientservlet" }, initParams = {
	...
	public void getIdDetails(HttpServletRequest request)
		...
		if (get.getStatusCode() == HttpStatus.SC_OK) {
			...
			try {
				...
				System.out
						.println("Creating DeploymentSettingClientDO : "
								+ orgId
								+ "~"
								+ accessToken
								+ "~"
		...
		try {
			...
			SFoAuthHandle sfBaseHandle = FDGetSFoAuthHandleService
					.getSFoAuthHandle(deploymentSettingsClientDO.getOrgId(),
							...
							deploymentSettingsClientDO.getServerUrl(), deploymentSettingsClientDO.getRefreshToken(), Constants.BaseOrgID);

			List deploymentSettingsClientList = deploymentSettingsClientDAO.findById(
					deploymentSettingsClientDO.getOrgId(), sfBaseHandle);
			// If should never occur
			if (deploymentSettingsClientList.size() == 0) {
				deploymentSettingsClientDAO
						.insert(deploymentSettingsClientDO, sfBaseHandle);

			} else {
				DeploymentSettingClientDO dsDO = null;
				for (Iterator iterator = deploymentSettingsClientList.iterator(); iterator
						.hasNext();) {
					dsDO = (DeploymentSettingClientDO) iterator
							.next();
					dsDO.setOrgId(deploymentSettingsClientDO
							.getOrgId());
					dsDO.setServerUrl(deploymentSettingsClientDO
							.getServerUrl());
					dsDO.setToken(deploymentSettingsClientDO
							.getToken());
					dsDO.setRefreshToken(deploymentSettingsClientDO
									.getRefreshToken());
					deploymentSettingsClientDAO.update(dsDO,
							sfBaseHandle);
				}
			}
			FDGetSFoAuthHandleService.setSfHandleToNUll();
